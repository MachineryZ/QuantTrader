...
**常见错误**1.谨慎地使用LastPrice。2.因子值极端化（无法用与阈值开仓平仓）3.长时间观察，应该是平稳。（用交易量去乘因子）4.使用到未来函数，rolling，shift中带了负号。填充nan用均值（应该用0）。用一天的数据去ZScore或者归一化。5.归一化的方法，用过去的一段时间的值去做ZScore，在保证因子平稳的前提下，用全局的标准差去归一化。tanh函数。6.因子计算提速方法，谨慎使用for，apply，bool索引改成numpy自带函数（多用np.where，而不是dataframe的mask）7.股票池的选择，
